...
const Header = () => {
  ...
  const { searchQuery, setSearchQuery, isOnline, pendingSync } = usePOSStore();
  ...
  return (
    <header className="bg-white border-b border-gray-200">
      {/* Offline banner */}
      {!isOnline && (
        <div className="flex items-center justify-center gap-2 bg-red-600 text-white text-sm py-1.5 px-4">
          <WifiOff className="w-4 h-4" />
          <span>Offline rejim — internet ulanishi yo'q</span>
          {pendingSync > 0 && (
      ...
      <div className="flex items-center justify-between h-16 px-6">
        ...
        <Link to="/" className="flex items-center space-x-3">
            <img 
              src="/assets/ury/pos/jazira_logo.png"
              alt="Jazira"
        ...
        <div className="flex items-center space-x-4">
          {/* Pending sync badge (online holatda) */}
          {isOnline && pendingSync > 0 && (
            <div className="flex items-center gap-1.5 bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-1 rounded-full border border-yellow-300">
              <CloudUpload className="w-3.5 h-3.5" />
              <span>{pendingSync} sync kutmoqda</span>
            </div>
          )}
          {/* User menu */}
