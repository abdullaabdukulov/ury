...
import { enqueuePayment, attachPaymentToOfflineOrder } from '../lib/offline-queue';
...
const PaymentDialog: React.FC<PaymentDialogProps> = ({
  ...
  useEffect(()=>{
    // Auto-fill the first payment mode (Naqd) with the full amount when dialog opens
    const firstMode = paymentModes[0];
    const firstModeId = typeof firstMode === 'string' ? firstMode : firstMode?.id;
    const otherPaymentModesNotEntered = Object.keys(paymentInputs).length <= 1;
    if (finalTotal && firstModeId && otherPaymentModesNotEntered) {
      setPaymentInputs((prev) => ({
        ...prev,
        [firstModeId]: String(finalTotal)
      }));
    }
  ...
  const handlePayment = async () => {
    ...
    setError(null);
    try {
      // Offline queue orderlar uchun — server API chaqirmasdan to'lovni queue'ga biriktirish
      if (invoice.startsWith('offline_')) {
        attachPaymentToOfflineOrder(invoice, {
          invoice,
          customer,
          cashier,
          owner,
          ...
          table,
          payments: payments as Array<{ mode_of_payment: string; amount: number }>,
          additionalDiscount: discountValue ? parseInt(discountValue) : null,
        });
        showToast.warn("To'lov saqlandi, buyurtma bilan birga internet kelganda sync bo'ladi");
        onClose();
        return;
      }
      ...
      // Tarmoq xatosi (frappe-sdk TypeError offline) — to'lovni queue'ga saqlash
      if (err instanceof TypeError) {
        ...
        showToast.warn("Offline: to'lov saqlandi, internet kelganda sync bo'ladi");
        onClose();
        clearSelectedOrder();
        await fetchOrders();
      } else {
