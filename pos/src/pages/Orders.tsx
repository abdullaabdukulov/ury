...
import { Clock, User, UserCheck, Printer, Pencil, X } from 'lucide-react';
  ...
  function printLocalReceipt() {
    ...
    const itemsHtml = items.map(item => `
      <tr>
        ...
        <td style="padding:3px 4px;text-align:center">${item.qty}</td>
        <td style="padding:3px 4px;text-align:right">${formatCurrency(item.amount)}</td>
      </tr>
    `).join('');
...
<style>
  body{font-family:monospace;font-size:13px;width:320px;margin:0 auto;padding:12px}
  h2{text-align:center;margin:0 0 8px}
  p{margin:2px 0}
  table{width:100%;border-collapse:collapse}
  th{text-align:left;border-bottom:1px solid #000;padding:3px 4px}
  .divider{border-top:1px dashed #000;margin:8px 0}
  .total td{border-top:2px solid #000;font-weight:bold;padding:4px}
</style></head>
<body>
  <h2>Receipt</h2>
  <p>Customer: ${order.customer}</p>
  <p>Date: ${order.posting_date} ${order.posting_time}</p>
  ${order.waiter ? `<p>Waiter: ${order.waiter}</p>` : ''}
  ${order.restaurant_table ? `<p>Table: ${order.restaurant_table}</p>` : ''}
  <div class="divider"></div>
  <table>
    <thead><tr><th>Item</th><th style="text-align:center">Qty</th><th style="text-align:right">Amount</th></tr></thead>
    <tbody>${itemsHtml}</tbody>
    <tfoot><tr class="total"><td colspan="2">Total</td><td style="text-align:right">${formatCurrency(order.rounded_total)}</td></tr></tfoot>
  </table>
  ...
  async function handlePrintOrder() {
    ...
    // Offline queue orders — server-side invoice yo'q, mahalliy chop etish
    if (selectedOrder._offline) {
      printLocalReceipt();
      return;
    }

    if (!posStore.posProfile) return;
    setIsPrinting(true);
    try {
      await printOrder({
      ...
      if (err instanceof TypeError) {
        // Server yetib bo'lmaydi — mahalliy chop etish
        printLocalReceipt();
      } else {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-screen-xl mx-auto">
              {orders.map((order) => (
                <Card
                  ...
                  } ${order._offline ? 'bg-yellow-50 border-yellow-300' : 'bg-white'}`}
                  onClick={() => handleOrderClick(order)}
                >
                  <CardContent className="p-0 flex flex-col h-full">
                    <div className={`p-3 border-b ${order._offline ? 'bg-yellow-100' : 'bg-gray-50'}`}>
                      <h3 className="font-medium text-gray-900 text-sm truncate" title={order.name}>
                        {order._offline ? 'Offline buyurtma' : order.name}
                      </h3>
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-500">{order.order_type}</p>
                        {order._offline ? (
                          <Badge variant="secondary" className="ml-2 bg-yellow-200 text-yellow-800 border-yellow-400">
                            Sync kutmoqda
                          </Badge>
                        ) : (
          ...
          <>
            ...
            <div className="sticky top-0 left-0 right-0 z-20 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between min-h-[64px]">
              ...
              <div className="flex items-center gap-2">
                ...
                {!selectedOrder._offline && (selectedOrder.status === 'Draft' || selectedOrder.status === 'Unbilled' || selectedOrder.status === 'Recently Paid') && (
                  <>
                    <button
                      type="button"
              ...
              <div className="flex items-center gap-3 w-full">
                ...
                {(selectedOrder._offline || selectedOrder.status === 'Draft' || selectedOrder.status === 'Unbilled' || selectedOrder.status === 'Recently Paid') && (
                  <Button
                    className="flex-1"
                    onClick={() => {
                      if (!selectedOrder._offline && String(selectedOrder.invoice_printed) === '0') {
