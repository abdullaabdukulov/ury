...
def sync_order(
    ...
    for d in items:
        ...
        # Fallback to the rate sent from POS if not found in Item Price
        rate = item_prices[0].price_list_rate if item_prices else d.get("rate")
    ...
    # table status
    if invoice.invoice_printed == 0 and table:
        frappe.db.set_value(
